...
class handle_file(object):
    ...
    def __init__(self, path, use_cache=True):
        ...
        self._mimetype = mimetypes.guess_type(path)[1]
    ...
    def __call__(self, environ, start_response):
        ...
        if not self._use_cache or self._cache is None or \
            ...
            with os.open(self._path, "r") as fp:
                self._cache = fp.read()
...
def handle_404(environ, start_response):
    ...
    return json.dumps({"error": 404, "message": "Not found."})
...
def handle_static(root, static_path="static"):
    def handle_file(environ, start_response):
        ...
        if not os.path.is_file(full_path):
            return handle_404(environ, start_response)
        handle_file(full_path, use_cache=False)(environ, start_response)
    return handle_file
